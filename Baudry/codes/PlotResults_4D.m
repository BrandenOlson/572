...
for kpdf=1:exp.res.BIC.K
    for i=1:101
        pdf(kpdf,:,i)=exp.res.BIC.p(kpdf)*mvnpdf([X,repmat(Y(i),101,1),repmat(mean(y(:,3)),101,1),repmat(mean(y(:,4)),101,1)],exp.res.BIC.mu(kpdf,:),exp.res.BIC.S(:,:,kpdf));
    end
end
...
for Kb=2:exp.res.BIC.K-1
    ...
    for k=1:K
        yl=y(find(exp.res.combi(K).labels(:,k)==1),1:2);
